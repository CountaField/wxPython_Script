...
from DailyProdExcelProcess import DailyReportProcess


class ProdDataImport(wx.Frame):
    def __init__(self, parent, id=-1 ):
        wx.Frame.__init__(self, parent, id, 'Daily_Production_Data_Import',size=(700,550))
        self.SetBackgroundColour('light gray')
        self.wildcard="xls files (*.xls)|*.xls"
        self.filename = ''
        menuBar = wx.MenuBar()
        menu = wx.Menu()
        ...
        if self.filename == '':
            ...
            multiLabel = wx.StaticText(parent=self.panel, id=-1, label='   Data Loading:', pos=(60, 60))
            self.multiText = wx.TextCtrl(parent=self.panel, id=-1, value='', size=(350, 200),
                                         style=wx.TE_MULTILINE | wx.TE_RICH2 | wx.TE_READONLY)
            loadLabel = wx.StaticText(parent=self.panel, id=-1, label='    Report:')
            self.loadText = wx.TextCtrl(parent=self.panel, id=-1, value='', size=(350, 200),
                                        style=wx.TE_MULTILINE | wx.TE_RICH2 | wx.TE_READONLY)
    ...
    def onOpen(self, event):
        ...
        dlg = wx.FileDialog(self, message="Open Input File...", defaultDir=r'W:\GSR\040000-Reservoir_Studies\040300-RE_Prod\Production Data\GSR production daily report\Daily report exchange',
        ...
        if self.filename is not '':
            ...
            self.multiText.AppendText('File Load Successful ! Now you Can Load Data!!!\n')
    ...
    def Transfer(self, event):
        ...
        self.dataprocess=DailyReportProcess(self.filename, r'W:\GSR\050000-GeoInformation\052300-Projects\Geoscience database\Input files\Production Data\Daily inputs')
        self.final_file=self.dataprocess.PassFile()
        if self.final_file!='':
            ...
            self.multiText.AppendText(self.final_file + '\n')
            end_point = self.multiText.GetInsertionPoint()
    ...
    def DataLoad(self, event):
        inputfile = open(
                r"W:\GSR\050000-GeoInformation\052300-Projects\User_Interface\wxPython_Script\Data_Process\Control_Files\Production_daily_report_import\Prod_data_sample.ctl",
                'r')
        outputfile = open(
                 r"W:\EXCHANGE\GSR_DATABASE_APPLICATION\Prod_data_import.CTL",
                'w')
        try:
            astring = inputfile.read()
            outputfile.write(astring.replace('$input', self.final_file))
        finally:
            inputfile.close()
            outputfile.close()
        logfile = r'W:\EXCHANGE\GSR_DATABASE_APPLICATION\\Prod_data_import.log'
        os.system(
            r'W:\GSR\050000-GeoInformation\052300-Projects\User_Interface\wxPython_Script\Data_Process\Control_Files\Production_daily_report_import\Run_me.bat')
        self.report(logfile)
...
if __name__ == '__main__':
    app = wx.PySimpleApp()
    frame = ProdDataImport(parent=None, id=-1)
    frame.Show()
    app.MainLoop()