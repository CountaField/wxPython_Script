...
class DesandingMain(wx.Notebook):
    def __init__(self,parent):
        wx.Notebook.__init__(self,parent,-1,style=wx.NB_BOTTOM)
        desand_summary=DesandReport(self)
        self.AddPage(desand_summary,'Desanding Report')



class DesandReport(ScrolledPanel.ScrolledPanel):
    def __init__(self,parent):
        super(DesandReport,self).__init__(parent)
    ...
    def ClusterWellInit(self):
        ...
        prod_date_cur = db.Sql(
            "select production_date from vw_desanding_table  group by production_date order by production_date desc ")
        proddate_list = []
        for id in prod_date_cur.fetchall():
            proddate_list.append(str(id[0]))
        wellid_cur = db.Sql("select wellid from vw_desanding_table  group by wellid order by wellid ")
        self.wellid_list = []
    ...
    def StaticBoxInitial(self):
        ...
        self.alert_message_1 = wx.CheckBox(self.filter_parameter_staticbox, -1, 'Desanding Amount ')
        self.alert_message_2 = wx.CheckBox(self.filter_parameter_staticbox, -1, 'Moving Equipment ')
    ...
    def AlarmGrid(self):
        self.par_string = "wellid,production_date,daily_prod,prod_time,desanding_log "
        self.col_list = self.par_string.upper().split(',')
        self.col_list.append('DESANDING_AMOUNT')
    ...
    def AlarmData(self):
        ...
        if well_id != '':
            ...
            if prod_date != '':
                ...
                cur = db.Sql(
                    "select " + self.par_string + "from vw_desanding_table where wellid in (" + well_id + ") and production_date=to_date(\'" + prod_date + "\',\'yyyy-mm-dd hh24:mi:ss\')")
                for id in cur.fetchall():
                    i = 0
                    for list_name in self.col_list:
                        if 'DESANDING_LOG' in list_name:
                            if 'desanding amount' in id[i]:
                                index=id[i].index('desanding amount')+len('desanding amount:')
                                astring=id[i][index:]
                                self.data[list_name].append(id[i][:index])
                                if 'desanding amount' in astring:
                                    index = astring.index('desanding amount') + len('desanding amount:')
                                    bstring=astring[index:]
                                    desanding_data_list.append(bstring)

                                else:
                                    desanding_data_list.append(astring)

                            else:
                                self.data[list_name].append(id[i])
                                desanding_data_list.append('0')
                        else:
                            self.data[list_name].append(id[i])
                        i += 1
                ...
                cur = db.Sql(
                    "select " + self.par_string + "from vw_desanding_table  where wellid in (" + well_id + ") order by production_date desc")
                for id in cur.fetchall():
                    i = 0
                    for list_name in self.col_list:
                        if 'DESANDING_LOG' in list_name:
            ...
            if prod_date != '':
                ...
                cur = db.Sql(
                    "select " + self.par_string + "from vw_desanding_table  where production_date=to_date(\'" + prod_date + "\',\'yyyy-mm-dd hh24:mi:ss\')")
                for id in cur.fetchall():
                    i = 0
                    for list_name in self.col_list:
                        if 'DESANDING_LOG' in list_name:
                ...
                cur = db.Sql(
                    "select " + self.par_string + "from vw_desanding_table  order by production_date desc")
                for id in cur.fetchall():
                    i = 0
                    for list_name in self.col_list[:-1]:
                        if 'DESANDING_LOG'in list_name:
                            if 'desanding amount' in id[i]:
                                index = id[i].index('desanding amount') + len('desanding amount:')
                                astring = id[i][index:]
        ...
        self.data['DESANDING_AMOUNT']=[]
        for id in desanding_data_list:
            self.data['DESANDING_AMOUNT'].append(id)
